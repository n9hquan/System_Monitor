...
static int read_proc_stat(int pid, char *name, int name_len,
                          unsigned long long *cpu_jiffies)
{
    ...
    int n = fscanf(f, "%d %255s %c %d %d %d %d %d %llu %llu %llu %llu %llu %llu %llu",
                   &dummy_int, tmp_name, &dummy_char,
                   &dummy_int, &dummy_int, &dummy_int, &dummy_int, &dummy_int,
                   &utime, &utime, &utime, &utime, &utime,
                   &utime, &stime);
    fclose(f);
    if (n < 15) return -1;

    char *start = strchr(tmp_name, '(');
    char *end   = strrchr(tmp_name, ')');
    if (start && end && end > start) {
        ...
        strncpy(name, tmp_name, name_len - 1);
        name[name_len - 1] = '\0';
    }
